...
class FirstIterator
{
    ...
    typedef T& reference;
    typedef T* pointer;
    typedef FirstIterator<T, Dims...> self;
    
    public:
    pointer node;
   // size_t dimensions[] = {Dims...};
    FirstIterator() : node(nullptr)  {}
    FirstIterator(const FirstIterator& rhs)
        : node(rhs.node) {}
   FirstIterator &
       operator=(const FirstIterator &rhs) 
   {
        node = rhs.node;
        return *this;
   }
...
template<typename T, size_t... Dims>
bool operator==(const FirstIterator<T,Dims...> &lhs,
    const FirstIterator<T,Dims...> &rhs)
{
...
class Array
{
    public:
    typedef  T ValueType;
    typedef  FirstIterator<T, Dims...> FirstDimensionMajorIterator;
...
class Array<T, D1>
{
    ...
    typedef  FirstIterator<T, D1> FirstDimensionMajorIterator;
